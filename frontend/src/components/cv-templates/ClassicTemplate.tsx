...
const ClassicTemplate = React.forwardRef<HTMLDivElement, ClassicTemplateProps>(({ t, c, colorMap }, ref) => {
    const blueAccent = colorMap[c] || "#00ABF0";

    return (
        <div
            ref={ref}
            id="cv-preview"
            className={`${t.backgroundColor} ${t.textColor} p-8 md:p-16 shadow-2xl min-w-[320px] font-sans`}
        >
            {/* Header */}
            <div className="text-center mb-10" data-cv-section="header">
                <h1 className={`text-4xl font-bold ${t.textColor} mb-2`}>{personalInfo.firstName} {personalInfo.lastName}</h1>
                <p className={`text-sm ${t.mutedTextColor} max-w-3xl mx-auto mb-6 leading-relaxed`}>
                ...
                <div className="flex flex-wrap justify-center items-center gap-6 text-sm">
                    <a href={`mailto:${personalInfo.email}`} className={`cv-link flex items-center gap-2 ${t.mutedTextColor} hover:text-blue-500 transition-colors`}>
                        <Mail size={16} style={{ color: blueAccent }} />
                        {personalInfo.email}
                    </a>
                    <a href={`tel:${personalInfo.phone.replace(/\s/g, '')}`} className={`cv-link flex items-center gap-2 ${t.mutedTextColor} hover:text-blue-500 transition-colors`}>
                        <Phone size={16} style={{ color: blueAccent }} />
                        {personalInfo.phone}
                    </a>
                    <a href="https://linkedin.com/in/dulshans" target="_blank" className={`cv-link flex items-center gap-2 ${t.mutedTextColor} hover:text-blue-500 transition-colors`}>
                        <Briefcase size={16} style={{ color: blueAccent }} />
                        LinkedIn
                    </a>
                    <a href="https://github.com/DulshanSiriwardhana" target="_blank" className={`cv-link flex items-center gap-2 ${t.mutedTextColor} hover:text-blue-500 transition-colors`}>
            ...
            <div className="mb-10" data-cv-section="experience">
                ...
                <div className={`h-0.5 w-full ${t.borderColor} opacity-50 mb-6`}></div>
                    ...
                    {experience.map((exp, index) => (
                        <div key={index} data-cv-section={`exp-${index}`}>
                            <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-2">
                                <div>
                                    <h3 className={`text-md font-bold ${t.textColor}`}>{exp.company}</h3>
                                    <p className={`text-sm font-medium ${t.mutedTextColor} italic`}>{exp.position}</p>
                                </div>
                                <span className={`text-xs font-bold ${t.mutedTextColor} opacity-60 mt-1 md:mt-0`}>{exp.duration}</span>
                            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4">
                    {skillCategories.map((cat, i) => (
                        <div key={i} className="flex flex-col">
                            <span className={`text-sm font-bold ${t.textColor} mb-1`}>{cat.category}:</span>
                            <span className={`text-sm ${t.mutedTextColor} leading-relaxed`}>{cat.skills.join(', ')}.</span>
                        </div>
                    ))}
                </div>
            </div>
